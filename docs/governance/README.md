...
| Audit artifacts | Lifecycle actions append JSONL events. Terminal handoff receipts pin manifest, ledger, and workspace snapshots by hash, and later flows verify those hashes. The lock, the initialize event, and the receipt also record whether the run opened under hook enforcement, and continuation refuses a source that did not. A public test checks the initialize ledger event, receipt hash pins, and a later Initialize that rejects a tampered ledger. | The ledger is helper-appended, not protected from arbitrary filesystem modification. Hash checks make some tampering detectable; they do not provide an external append-only store. The enforcement record is open-time truth, not proof that every write in the run was policed. |
| Session-bound lock | `Get-ActiveLockForSession` refuses `StartTask`, `FinishTask`, `RecordRetry`, and `Complete` when the lock's `enforcement.mode` is not `none` and `$env:CLAUDE_CODE_SESSION_ID` is not the recorded `enforcement.session_id`. A public test covers same-session advance, a plain session, a different governed session, and `mode = 'none'`. | `RecoverLock` stays operator-driven and does not go through this check. The helper `-SessionId` is agent-supplied and is not the identity, but `CLAUDE_CODE_SESSION_ID` is an environment variable a shell command can set, and the lock records the owning session in plain text. This catches a different session picking up a lock, not one impersonating the owner on purpose. The liveness marker is not re-read per action. |
